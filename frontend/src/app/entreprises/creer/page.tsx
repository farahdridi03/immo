...
export default function CreerEntreprisePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newEnt = await entreprisesApi.create(payload);
      if (newEnt && newEnt.id) {
        router.push(`/entreprises/${newEnt.id}`);
      } else {
        router.push("/entreprises");
      }
    } catch (err: any) {
      console.error("Failed to create entreprise", err);
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-[#E0DACB] p-6 shadow-2xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <div className="h-12 w-12 rounded-xl bg-[#EAE3CE] text-[#1C1917] font-bold flex items-center justify-center shrink-0 shadow-2xs border border-[#E0DACB]">
            ...
            <div>
              ...
              <p className="text-xs text-[#78716C]">
                Création et enregistrement d'une nouvelle entité morale dans le système.
              </p>
            </div>
          </div>
        </div>
            ...
            <CardContent className="pt-6 space-y-6">
              ...
              <div className="pt-4 border-t border-[#E0DACB]/60 flex items-center justify-end gap-3">
                ...
                >
                  ...
                  {saving ? "Création en cours..." : "Enregistrer l'entreprise"}
                </Button>
              </div>
            </CardContent>
          </Card>
